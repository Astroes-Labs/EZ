...
{
    // Coin Value function
    public function getCoinValue($symbol)
    {
        $apiKey = config('services.financial_data.api_key');
        $response = Http::get("https://financialmodelingprep.com/api/v3/quote/{$symbol}?apikey={$apiKey}");

        if ($response->successful() && !empty($response[0]['price'])) {
